...
class_tif = require_type('hh::fw::AppModule')

class_class_name = ['AppModule', 'fw', 'hh']

# def handle_app_module_ctor(instantiator_thunk, instantiator_func, ctor_thunk, ctor_func, base_ctor_func):
#     class_ea = instantiator_thunk and find_class_object('AppModule', rdata_seg, 0x18, instantiator_thunk)

#     class_name, using_fallback_name = get_best_class_name(ctor_func, class_ea and get_qword(class_ea), 'app_modules')

#     print(f'info: handling app module at {ctor_func.start_ea:x}: {friendly_class_name(class_name)}')
...
#     if class_ea == None:
#         print(f'warn: Could not find a reliable hh::fnd::AppModule xref for constructor {ctor_thunk.start_ea:x} (instantiator thunk was {instantiator_thunk.start_ea if instantiator_thunk else 0:x}). Constructor name has been deduced through vtable.')
#         return
    
#     force_apply_tinfo(class_ea, class_tif)

#     class_var = StaticObjectVar('appModuleClass', class_class_name, StaticObjectVarType.VAR, True)
...
#     else:
#         print(f'warn: no GetClass function found for AppModule at {class_ea:x}')

#     if using_fallback_name:
#         set_generated_vtable_name_through_ctor(ctor_func, class_name)

# def find_managed_app_modules():
#     base_ctor_ea = require_name_ea('??AppModule@fw@hh@@QEAA@PEAVIAllocator@fnd@csl@@@Z')
#     base_ctor = require_function(base_ctor_ea)

#     for funcs in discover_class_hierarchy(base_ctor):
#         handle_anal_exceptions(lambda: handle_app_module_ctor(*funcs))
