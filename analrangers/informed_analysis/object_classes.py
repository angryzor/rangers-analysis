...
from analrangers.lib.funcs import ensure_functions, set_func_name, find_implementation
...
def handle_obj_class(obj_class_ea):
    ...
    set_name(member_value_array_ea, f'?staticClassMembers@{class_name}@@0PEAVValue@RflClassMember@fnd@hh@@EA')

    set_name(obj_class_ea, f'?staticClass@{class_name}@@0PEAVGameObjectClass@game@hh@@EA')
    ...
    if instantiator != constructor_thunk:
        set_func_name(constructor_thunk, f'??0{class_name}@@AEAA@PEAVIAllocator@fnd@csl@@@Z')
